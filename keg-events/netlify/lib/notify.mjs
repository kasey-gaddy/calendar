...
export const manageUrl = (type, e, t) =>
  `${siteUrl()}/manage.html?type=${type}&e=${encodeURIComponent(e)}&t=${t}`;
...
export function emailHtml({ heading, intro, items = [], note, ctaUrl, ctaLabel = "View event", footer }) {
  const list = items.length
    ? `<ul style="margin:0 0 20px;padding-left:20px">${items
        ...
        .join("")}</ul>`
    : "";
  return `<!doctype html><html><body style="margin:0;background:#F2F4F7;font-family:Arial,Helvetica,sans-serif;color:#1A2744">
...
<tr><td style="background:#0D1F3C;border-bottom:5px solid #E8621A;padding:16px 24px;color:#ffffff;font-size:20px;font-weight:bold">KE&amp;G Events</td></tr>
<tr><td style="padding:24px">
<h1 style="margin:0 0 12px;font-size:24px;line-height:1.2;color:#0D1F3C">${h(heading)}</h1>
${intro ? `<p style="margin:0 0 16px;font-size:16px;line-height:1.5">${h(intro)}</p>` : ""}
${list}
${note ? `<p style="margin:0 0 20px;padding:12px 14px;background:#FFF4EC;border-left:4px solid #E8621A;font-size:15px;line-height:1.5">${h(note)}</p>` : ""}
${ctaUrl ? `<a href="${ctaUrl}" style="display:inline-block;background:#E8621A;color:#ffffff;text-decoration:none;font-weight:bold;padding:12px 20px;border-radius:6px">${h(ctaLabel)}</a>` : ""}
</td></tr>
