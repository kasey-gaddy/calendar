window.KEG = (() => {
  ...
  const PALETTE = ["#233e87", "#C2622D", "#2F7D5B", "#6B4FA0", "#1F7A8C", "#9A7B1C", "#B03A48", "#5B6B8C"];
  ...
  function calLinks(ev, feedToken) {
    ...
    return {
      ...
      ics: "/api/event.ics?" + qs({ id: ev.id, t: feedToken || "" }),
    };
  }
  ...
  async function api(path, opts = {}) {
    ...
    const token = local.get("bdlSession");
    if (token && !opts.asAdmin) headers["x-session"] = token;
    const adminKey = opts.asAdmin ? session.get("bdlAdmin") : null;
  ...
  // Fire-and-forget activity tracking. keepalive lets it finish even if the
  // click opens a new page.
  function track(eventId, action) {
    const token = local.get("bdlSession");
    if (!token) return;
    try {
      fetch("/api/track", {
  ...
  return {
    ...
    whenText, timeText, money, costText, catColor, setCategories, linkify, calLinks, api, local, session, toast, track,
  };
})();
